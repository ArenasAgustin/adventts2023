 ...
 * //   8  12     1
 *
 * Lo que necesitamos es transformar el array en un objeto donde cada nodo del árbol tiene
 * las propiedades value, left y right.
 *
 * Por ejemplo, al ejecutar tu función transformTree con [3, 1, 0, 8, 12, null, 1] debería
 * devolver esto:
 *
 * {
 *   value: 3,
...
function transformTree(tree: Array<number | null>): TreeNode | null {
  const setNode = (index: number): TreeNode | null => {
    if (index >= tree.length || tree[index] === null) return null;
    return {
      value: tree[index] as number,
      left: setNode(index * 2 + 1),
      right: setNode(index * 2 + 2),
    };
  };

  const root = setNode(0);
  return root;
}
